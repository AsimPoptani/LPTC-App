...
function findAllUnansweredQuestions(database:Dictionary<any>):Array<Question>
{
...
function expressionCheck(database:Dictionary<any>,item:(Question|Recommendation|Benefit|Expression)):boolean {
    ...
    if (item instanceof Expression){
        ...
        expression=expressions.find(expression=>expression.id==item.expressionId);
    }

    // Lets check if there are any expressionExpressions and answerQuestion
    let expressionExpressions: Array<ExpressionExpression> = database.expressionExpressions;
    let answerExpressions: Array<AnswerExpression> = database.answerExpressions;
    let answers: Array<Answer> = database.answers;
    
    expressionExpressions=expressionExpressions.filter(expressionExpression => expressionExpression.expressionId1==expression.id || expressionExpression.expressionId2==expression.id)
    answerExpressions=answerExpressions.filter(answerExpression => answerExpression.expressionId==expression.id)
    ...
    if (expression.isAnd){
        ...
        for (let expressionExpression of expressionExpressions){
            ...
            if (expressionExpression.expressionId1==expression.id){
                expressionNot=expressions.find(exp=>exp.id==expressionExpression.expressionId2);
            }
            else{
                expressionNot=expressions.find(exp=>exp.id==expressionExpression.expressionId1);
            }
        ...
        for (let answerExpression of answerExpressions){
            let answer=answers.find(answer=>answerExpression.answerId==answer.id);
            if (!answer.state){
                return false
            }
        }
...
function findAllQuestionsExpressionsFulfilled(database:Dictionary<any>):Array<Question>
{
...
// All questions which have the Expressions Fulfilled 
function findAllUnansweredQuestionsExpressionsFulfilled(database:Dictionary<any>):Array<Question>
{
    let questionsExpressionsFulfilled: Array<Question> = []
    let questions: Array<Question>= database.questions
    // Check if expressions are fullfilled.
    for (let question of questions){
        if(question.answered&&(question.expressionId == null || expressionCheck(database,question))){
            questionsExpressionsFulfilled.push(question)
        }
    }
    return questionsExpressionsFulfilled; 
...
export {findAllUnansweredQuestions,findAllQuestionsExpressionsFulfilled,expressionCheck,findAllUnansweredQuestionsExpressionsFulfilled}
